...
import { Plus, Trash2, Copy, Loader2, AlertCircle, Send, Key } from "lucide-react";
...
export default function InstructionBuilder() {
  ...
  const packInstruction = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/solana/instruction/pack`, {
        method: "POST",
      ...
      console.error(err);
      setPackError(err.message || "Error packing data");
    } finally {
      setIsPacking(false);
    }
  }, [fields]);

  // Auto-pack when fields change
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8">
        ...
        {/* STEP 1: Instruction Data Builder */}
        <div>
          <div className="mb-4">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Step 1: Build Instruction Data</h2>
            <p className="text-muted-foreground mt-1">Manually pack bytes for your instruction.</p>
          </div>
          ...
          {/* Packed Output */}
          <div className="mt-4 bg-black border border-border rounded-lg p-4 font-mono text-sm min-h-[100px] relative">
            ...
            <div className="break-all leading-relaxed tracking-wide text-green-400/90 pr-12">
              {packError ? (
                <span className="text-red-400 flex items-center gap-2">
                  <AlertCircle className="h-4 w-4" /> {packError}
                </span>
              ) : (
                bufferHex || <span className="text-muted-foreground opacity-50">Instruction data will appear here...</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div>
          <div className="mb-4">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Step 2: Configure Transaction</h2>
            <p className="text-muted-foreground mt-1">Specify the program and accounts.</p>
          </div>
        ...
        <div>
          <div className="mb-4">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Step 3: Execute Transaction</h2>
            <p className="text-muted-foreground mt-1">Build, sign, and send your transaction.</p>
          </div>
